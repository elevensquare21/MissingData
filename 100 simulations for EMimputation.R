...
for (k in 1:10){
	...
	#reduce minute level data to daily summary data
	...
	simag$miss=ifelse(simag$prop>0.5,1,0)
	simag$activity[which(simag$miss==1)]=NA
	simag=simag[,c(1,2,3)]
	...
	simag=simag[,c(2,3,4,6,7,8,9)]
	a.out=amelia(simag,m=1,ts='day',cs='identifier')
	simag.imp=a.out[[1]]$imp1
	simag.imp$activity=ifelse(simag.imp$activity<0,0,simag.imp$activity)
	
	#perform regression
	nvmodel=lme(activity~age+bmi+depression+ind,random=~1|identifier,simag.imp,method='REML')
...
m0=rbind(sim1[[1]],sim2[[1]],sim3[[1]],sim4[[1]])	
...
save(model.imp,file='modelimp result.rdata')
