...
for (k in 1:10){
	...
	#reduce minute level data to daily summary data and run method 1: weighted regression (with 5 different weighing schemes)
	...
	nvmodel=lme(activity~age+bmi+depression+ind,random=~1|identifier,simag,method='REML')
	...
	#reduce minute level data to daily summary data and run method 2: imputed sum (with 3 different mixed model approaches)
	simaggsum=aggregate(simd$activity,list(simd$dt,simd$identifier),sum,na.rm=TRUE)
	names(simaggsum)=c('dt','identifier','activitysum')
	simagg$activitysum=simaggsum$activitysum
	simag=na.omit(simagg)
	simag$weart=1440*(1-simag$prop)
	simm=lme(activitysum~weart,random=~1|identifier,simag,method='REML')
	...
	act=diff*slope
	bad$activitysum=bad$activitysum+act
	bad$weart=720
	bad$activity=bad$activitysum/720
	newag=rbind(good,bad)
	
	newag$identifier=as.character(newag$identifier)
	newag$identifier=substr(newag$identifier,4,15)
	for(i in 1:nrow(newag)){
		ma=match(newag$identifier[i],nv[,1])
		newag$age[i]=nv[ma,3]
		newag$bmi[i]=nv[ma,2]
		newag$depression[i]=nv[ma,4]
	}
	newag$depression=ifelse(newag$depression=="Don't Know", NA,newag$depression)
	newag$depression=as.factor(newag$depression)
	newag$dt=as.Date(newag$dt)
	newag$dow=weekdays(newag$dt)
	newag$ind=ifelse(newag$dow=='Saturday'|newag$dow=='Sunday',1,0)
	newag$ind=as.factor(newag$ind)

	newag=na.omit(newag)
	
	mod=lme(activity~age+bmi+depression+ind,random=~1|identifier,newag,method='REML')
	...
	good=simag[simag$prop<0.5,]
	bad=simag[simag$prop>=0.5,]
	simm2=lme(activitysum~weart,random=~1+weart|identifier,simag,method='REML')
	for(i in 1:nrow(bad)){
		name=bad[i,2]
		...
		diff=720-bad[i,6]
		act=diff*slope
		bad$activitysum[i]=bad$activitysum[i]+act
		bad$weart[i]=720
		bad$activity[i]=bad$activitysum[i]/720
	}
	newag=rbind(good,bad)
	newag$identifier=as.character(newag$identifier)
	newag$identifier=substr(newag$identifier,4,15)
	...
	mod2=lme(activity~age+bmi+depression+ind,random=~1|identifier,newag,method='REML')
	...
	good=simag[simag$prop<0.5,]
	bad=simag[simag$prop>=0.5,]
	simm3=lme(activitysum~weart-1,random=~weart-1|identifier,simag,method='REML')
	for(i in 1:nrow(bad)){
		name=bad[i,2]
	...
	mod3=lme(activity~age+bmi+depression+ind,random=~1|identifier,newag,method='REML')
...
model1[1,1]=sqrt(sum((m1[,4]+4.6003)^2)/100)
model1[1,2]=mean(m1[,4])+4.6003
model1[1,3]=sqrt((model1[1,1])^2-(model1[1,2])^2)
model1[1,4]=sum(ifelse(m1[,4]<0 & m1[,6]<0.01,1,0))/100
model1[2,1]=sqrt(sum((m1[,7]+4.1768)^2)/100)
model1[2,2]=mean(m1[,7])+4.1768
model1[2,3]=sqrt((model1[2,1])^2-(model1[2,2])^2)
model1[2,4]=sum(ifelse(m1[,7]<0 & m1[,9]<0.01,1,0))/100
model1[3,1]=sqrt(sum((m1[,10]+30.9295)^2)/100)
model1[3,2]=mean(m1[,10])+30.9295
model1[3,3]=sqrt((model1[3,1])^2-(model1[3,2])^2)
model1[3,4]=sum(ifelse(m1[,10]<0 & m1[,12]<0.01,1,0))/100
model1[4,1]=sqrt(sum((m1[,13]-8.3622)^2)/100)
model1[4,2]=mean(m1[,13])-8.3622
model1[4,3]=sqrt((model1[4,1])^2-(model1[4,2])^2)
model1[4,4]=sum(ifelse(m1[,13]>0 & m1[,15]<0.1,1,0))/100
...
model2[1,1]=sqrt(sum((m2[,4]+4.6003)^2)/100)
model2[1,2]=mean(m2[,4])+4.6003
model2[1,3]=sqrt((model2[1,1])^2-(model2[1,2])^2)
model2[1,4]=sum(ifelse(m2[,4]<0 & m2[,6]<0.01,1,0))/100
model2[2,1]=sqrt(sum((m2[,7]+4.1768)^2)/100)
model2[2,2]=mean(m2[,7])+4.1768
model2[2,3]=sqrt((model2[2,1])^2-(model2[2,2])^2)
model2[2,4]=sum(ifelse(m2[,7]<0 & m2[,9]<0.01,1,0))/100
model2[3,1]=sqrt(sum((m2[,10]+30.9295)^2)/100)
model2[3,2]=mean(m2[,10])+30.9295
model2[3,3]=sqrt((model2[3,1])^2-(model2[3,2])^2)
model2[3,4]=sum(ifelse(m2[,10]<0 & m2[,12]<0.01,1,0))/100
model2[4,1]=sqrt(sum((m2[,13]-8.3622)^2)/100)
model2[4,2]=mean(m2[,13])-8.3622
model2[4,3]=sqrt((model2[4,1])^2-(model2[4,2])^2)
model2[4,4]=sum(ifelse(m2[,13]>0 & m2[,15]<0.1,1,0))/100
...
model3[1,1]=sqrt(sum((m3[,4]+4.6003)^2)/100)
model3[1,2]=mean(m3[,4])+4.6003
model3[1,3]=sqrt((model3[1,1])^2-(model3[1,2])^2)
model3[1,4]=sum(ifelse(m3[,4]<0 & m3[,6]<0.01,1,0))/100
model3[2,1]=sqrt(sum((m3[,7]+4.1768)^2)/100)
model3[2,2]=mean(m3[,7])+4.1768
model3[2,3]=sqrt((model3[2,1])^2-(model3[2,2])^2)
model3[2,4]=sum(ifelse(m3[,7]<0 & m3[,9]<0.01,1,0))/100
model3[3,1]=sqrt(sum((m3[,10]+30.9295)^2)/100)
model3[3,2]=mean(m3[,10])+30.9295
model3[3,3]=sqrt((model3[3,1])^2-(model3[3,2])^2)
model3[3,4]=sum(ifelse(m3[,10]<0 & m3[,12]<0.01,1,0))/100
model3[4,1]=sqrt(sum((m3[,13]-8.3622)^2)/100)
model3[4,2]=mean(m3[,13])-8.3622
model3[4,3]=sqrt((model3[4,1])^2-(model3[4,2])^2)
model3[4,4]=sum(ifelse(m3[,13]>0 & m3[,15]<0.1,1,0))/100
...
model4[1,1]=sqrt(sum((m4[,4]+4.6003)^2)/100)
model4[1,2]=mean(m4[,4])+4.6003
model4[1,3]=sqrt((model4[1,1])^2-(model4[1,2])^2)
model4[1,4]=sum(ifelse(m4[,4]<0 & m4[,6]<0.01,1,0))/100
model4[2,1]=sqrt(sum((m4[,7]+4.1768)^2)/100)
model4[2,2]=mean(m4[,7])+4.1768
model4[2,3]=sqrt((model4[2,1])^2-(model4[2,2])^2)
model4[2,4]=sum(ifelse(m4[,7]<0 & m4[,9]<0.01,1,0))/100
model4[3,1]=sqrt(sum((m4[,10]+30.9295)^2)/100)
model4[3,2]=mean(m4[,10])+30.9295
model4[3,3]=sqrt((model4[3,1])^2-(model4[3,2])^2)
model4[3,4]=sum(ifelse(m4[,10]<0 & m4[,12]<0.01,1,0))/100
model4[4,1]=sqrt(sum((m4[,13]-8.3622)^2)/100)
model4[4,2]=mean(m4[,13])-8.3622
model4[4,3]=sqrt((model4[4,1])^2-(model4[4,2])^2)
model4[4,4]=sum(ifelse(m4[,13]>0 & m4[,15]<0.1,1,0))/100
...
rmodel1[1,1]=sqrt(sum((rm1[,4]+4.6003)^2)/100)
rmodel1[1,2]=mean(rm1[,4])+4.6003
rmodel1[1,3]=sqrt((rmodel1[1,1])^2-(rmodel1[1,2])^2)
rmodel1[1,4]=sum(ifelse(rm1[,4]<0 & rm1[,6]<0.01,1,0))/100
rmodel1[2,1]=sqrt(sum((rm1[,7]+4.1768)^2)/100)
rmodel1[2,2]=mean(rm1[,7])+4.1768
rmodel1[2,3]=sqrt((rmodel1[2,1])^2-(rmodel1[2,2])^2)
rmodel1[2,4]=sum(ifelse(rm1[,7]<0 & rm1[,9]<0.01,1,0))/100
rmodel1[3,1]=sqrt(sum((rm1[,10]+30.9295)^2)/100)
rmodel1[3,2]=mean(rm1[,10])+30.9295
rmodel1[3,3]=sqrt((rmodel1[3,1])^2-(rmodel1[3,2])^2)
rmodel1[3,4]=sum(ifelse(rm1[,10]<0 & rm1[,12]<0.01,1,0))/100
rmodel1[4,1]=sqrt(sum((rm1[,13]-8.3622)^2)/100)
rmodel1[4,2]=mean(rm1[,13])-8.3622
rmodel1[4,3]=sqrt((rmodel1[4,1])^2-(rmodel1[4,2])^2)
rmodel1[4,4]=sum(ifelse(rm1[,13]>0 & rm1[,15]<0.1,1,0))/100
...
rmodel2[1,1]=sqrt(sum((rm2[,4]+4.6003)^2)/100)
rmodel2[1,2]=mean(rm2[,4])+4.6003
rmodel2[1,3]=sqrt((rmodel2[1,1])^2-(rmodel2[1,2])^2)
rmodel2[1,4]=sum(ifelse(rm2[,4]<0 & rm2[,6]<0.01,1,0))/100
rmodel2[2,1]=sqrt(sum((rm2[,7]+4.1768)^2)/100)
rmodel2[2,2]=mean(rm2[,7])+4.1768
rmodel2[2,3]=sqrt((rmodel2[2,1])^2-(rmodel2[2,2])^2)
rmodel2[2,4]=sum(ifelse(rm2[,7]<0 & rm2[,9]<0.01,1,0))/100
rmodel2[3,1]=sqrt(sum((rm2[,10]+30.9295)^2)/100)
rmodel2[3,2]=mean(rm2[,10])+30.9295
rmodel2[3,3]=sqrt((rmodel2[3,1])^2-(rmodel2[3,2])^2)
rmodel2[3,4]=sum(ifelse(rm2[,10]<0 & rm2[,12]<0.01,1,0))/100
rmodel2[4,1]=sqrt(sum((rm2[,13]-8.3622)^2)/100)
rmodel2[4,2]=mean(rm2[,13])-8.3622
rmodel2[4,3]=sqrt((rmodel2[4,1])^2-(rmodel2[4,2])^2)
rmodel2[4,4]=sum(ifelse(rm2[,13]>0 & rm2[,15]<0.1,1,0))/100
...
rmodel3[1,1]=sqrt(sum((rm3[,4]+4.6003)^2)/100)
rmodel3[1,2]=mean(rm3[,4])+4.6003
rmodel3[1,3]=sqrt((rmodel3[1,1])^2-(rmodel3[1,2])^2)
rmodel3[1,4]=sum(ifelse(rm3[,4]<0 & rm3[,6]<0.01,1,0))/100
rmodel3[2,1]=sqrt(sum((rm3[,7]+4.1768)^2)/100)
rmodel3[2,2]=mean(rm3[,7])+4.1768
rmodel3[2,3]=sqrt((rmodel3[2,1])^2-(rmodel3[2,2])^2)
rmodel3[2,4]=sum(ifelse(rm3[,7]<0 & rm3[,9]<0.01,1,0))/100
rmodel3[3,1]=sqrt(sum((rm3[,10]+30.9295)^2)/100)
rmodel3[3,2]=mean(rm3[,10])+30.9295
rmodel3[3,3]=sqrt((rmodel3[3,1])^2-(rmodel3[3,2])^2)
rmodel3[3,4]=sum(ifelse(rm3[,10]<0 & rm3[,12]<0.01,1,0))/100
rmodel3[4,1]=sqrt(sum((rm3[,13]-8.3622)^2)/100)
rmodel3[4,2]=mean(rm3[,13])-8.3622
rmodel3[4,3]=sqrt((rmodel3[4,1])^2-(rmodel3[4,2])^2)
rmodel3[4,4]=sum(ifelse(rm3[,13]>0 & rm3[,15]<0.1,1,0))/100
